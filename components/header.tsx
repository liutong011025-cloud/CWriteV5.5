...
export default function Header() {
  ...
  const navItems = [
    ...
    { label: "CWrite", href: "/cwrite", action: () => {
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('navigateToCwrite'))
      }
      window.scrollTo({ top: 0, behavior: 'smooth' })
    } },
  ...
  const isCwritePage = currentStage === 'cwrite' || pathname === '/cwrite'
  ...
  const showBackground = isScrolled || isAboutPage || isGalleryPage || isCwritePage
  const showLogo = isHomePage && isAtTop && !isHovering && !isAboutPage && !isGalleryPage
              ...
              />
               ...
               <nav className="flex items-center gap-3">
                 {navItems.map((item) => {
                  ...
                  const isCwritePageActive = (currentStage === 'cwrite' || pathname === '/cwrite') && item.href === "/cwrite"
                  ...
                  const isActive = isHomePageActive || isCwritePageActive || isAboutPageActive || isWritePageActive || isGalleryPageActive
                   return (
                     <Link
                       key={item.href}
                       href={item.href}
