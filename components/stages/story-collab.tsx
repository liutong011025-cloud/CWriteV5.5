...
export default function StoryCollab({
  ...
  const sendMessage = useCallback(
    async (text: string, action?: "help_me" | "chat" | "submit_section") => {
      ...
      setIsLoading(true)
      trackProcess(
        action === "help_me" ? "STORY_018" : action === "submit_section" ? "STORY_024" : "STORY_016",
        {
          message: text.slice(0, 8000),
          section_index: currentWritingSection,
          action: action || "chat",
        },
        { stage: "storyCollab", phase },
      )
      ...
      try {
        ...
        trackProcess(
          "STORY_AI_OUTPUT",
          {
            answer: (cleaned || rawAnswer).slice(0, 4000),
            section_passed: data.section_passed === true,
            phase: data.phase || phase,
          },
          { stage: "storyCollab", phase: data.phase || phase },
        )
  ...
  const openStructureSelection = useCallback(
    (userLine?: string) => {
      ...
      setChatInput("")
      setPhase("structure")
      trackProcess("STORY_019", { plot_state: nextPlot }, { stage: "storyCollab", phase: "structure" })
    },
    [
      selectedStructure,
      plotComplete,
  ...
  const handleNextSection = useCallback(() => {
    ...
    trackProcess("STORY_026", { section_index: idx, text_snapshot: merged.slice(0, 8000) }, { stage: "storyCollab", phase: "writing" })
    trackProcess("STORY_028", { from_section: idx, to_section: idx + 1 }, { stage: "storyCollab", phase: "writing" })
  ...
  const handleLastSectionContinue = useCallback(() => {
    ...
    trackProcess("STORY_027", { section_index: idx, text_snapshot: merged.slice(0, 8000) }, { stage: "storyCollab", phase: "writing" })
  }, [currentWritingSection, storyBlocks, chatInput, t])
  ...
  const handleSuggestionClick = useCallback(
    (suggestion: string) => {
      ...
      if (storyBlocks.length > 0 && mode === "ai") {
        ...
        trackProcess("STORY_017", { suggestion_text: englishOption, section_index: currentWritingSection }, { stage: "storyCollab", phase })
        return
      }
      updateWritingMoodFromText(englishOption)
      trackProcess("STORY_017", { suggestion_text: englishOption }, { stage: "storyCollab", phase })
  ...
  const handleStructureSelect = useCallback(
    (type: StructureType) => {
      ...
      trackProcess("STORY_020", { structure_type: type }, { stage: "storyCollab", phase: "structure" })
    },
  ...
  const handleStoryBlockChange = useCallback(
    (index: number, text: string) => {
      ...
      trackProcessInput("STORY_021", text, { section_index: index }, { stage: "storyCollab", phase: "writing" })
    },
  ...
  const handleFinishStory = useCallback(() => {
    ...
    trackProcess(
      "STORY_029",
      { word_count: totalWords, full_text: finalStory.slice(0, 8000) },
      { stage: "storyCollab", phase: "writing" },
    )
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <StageHeader
          ...
          title={t.writeTitle}
          onBack={() => {
            trackProcess("STORY_040", { from_stage: "storyCollab", phase }, { stage: "storyCollab", phase })
            onBack()
          }}
                          ...
                          <Textarea
                            ...
                            onKeyDown={(e) => {
                              if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
                                e.preventDefault()
                                trackProcess("STORY_025", { section_index: currentWritingSection }, { stage: "storyCollab", phase })
                                handleSendChat()
                              }
                            }}
                            placeholder={
