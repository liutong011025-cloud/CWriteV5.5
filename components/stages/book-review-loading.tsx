...
export default function BookReviewLoading({
  ...
  return (
    <PixelPage style={{ paddingTop: "120px", paddingBottom: "120px" }}>
      <div className="max-w-6xl mx-auto relative flex flex-col pl-16 lg:pl-20">
        {onBack && <BackButton onClick={onBack} variant="blue" />}

        <div className="text-center mb-6">
          <h1 className="text-3xl md:text-4xl font-black mb-2 pixel-title" style={{ color: "#6b5210", textShadow: "2px 2px 0 rgba(0,0,0,0.15)" }}>
            Preparing Your Writing Space
          </h1>
          <p className="text-base pixel-text mb-2" style={{ color: "#5a4a2a" }}>
            Reviewing: <strong>{bookTitle}</strong>
          </p>
        </div>

        <div className="max-w-4xl mx-auto flex-1 flex items-start justify-center py-8">
          {/* 写作秘诀 - 向右填充，占据更大空间 */}
          <div className="pixel-panel p-8 shadow-2xl relative w-full">
            {/* 加载转圈 - 放在右上角 */}
            <div className="absolute top-6 right-6">
              <Loader2 className="w-10 h-10 animate-spin" style={{ color: "#5a9a32" }} />
            </div>

            <h2 className="text-2xl font-bold mb-6 pr-16 pixel-title" style={{ color: "#6b5210" }}>
              {tips.title}
            </h2>
            <div className="space-y-3 pr-16">
              {tips.tips.map((tip, index) => (
                <div
                  key={index}
                  className="pixel-card p-4 shadow-lg animate-fade-in-up"
                  style={{ animationDelay: `${index * 0.1}s` }}
                >
                  <p className="text-base leading-relaxed pixel-text" style={{ color: "#5a4a2a" }}>
                    {tip}
                  </p>
                </div>
              ))}
            ...
            {/* 结构预览 - 显示正常顺序 */}
            <div className="mt-6 pixel-card p-5 shadow-lg pr-16">
              <h3 className="text-lg font-bold mb-3 pixel-text" style={{ color: "#6b5210" }}>Your Review Sections 📦:</h3>
              <div className="space-y-2">
                {(() => {
                  ...
                  return sectionsToDisplay.map((section, index) => {
                    ...
                    return (
                      <div key={index} className="flex items-center gap-3 pixel-text" style={{ color: "#5a4a2a" }}>
                        <span className="w-7 h-7 flex items-center justify-center font-bold flex-shrink-0 text-sm" style={{ background: "#e8c547", border: "3px solid #8b6914", color: "#5a4a2a" }}>
                          📦
                        </span>
