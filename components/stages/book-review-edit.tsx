...
export default function BookReviewEdit({
  ...
  const handleSave = async () => {
    trackProcess("BOOK_039", { edited_review_text: editedReview.slice(0, 8000) }, { stage: "bookReviewEdit" })
    setIsSaving(true)
    try {
      // 保存到数据库
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <Button
            onClick={onBack}
            variant="outline"
            className="mb-4 pixel-btn pixel-btn-wood"
          >
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
          <h1 className="text-4xl font-black mb-2 pixel-title" style={{ color: "#6b5210", textShadow: "2px 2px 0 rgba(0,0,0,0.15)" }}>
            Edit Your Review ✏️
          </h1>
          <p className="pixel-text" style={{ color: "#5a4a2a" }}>Make changes to your book review. AI will provide helpful suggestions!</p>
        </div>

        <div className="grid lg:grid-cols-3 gap-6">
          {/* 左侧：书评信息 */}
          <div className="lg:col-span-1 space-y-4">
            <div className="pixel-panel p-6 shadow-lg">
              <h3 className="text-xl font-bold mb-4 pixel-text" style={{ color: "#6b5210" }}>Review Info</h3>
              <div className="space-y-3">
                <div>
                  <p className="text-sm font-semibold mb-1 pixel-text" style={{ color: "#6b5210" }}>Type</p>
                  <p className="text-lg font-bold pixel-text" style={{ color: "#3a8aa3" }}>{reviewTypeNames[reviewType]}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold mb-1 pixel-text" style={{ color: "#6b5210" }}>Book Title</p>
                  <p className="text-lg font-bold italic pixel-text" style={{ color: "#5a4a2a" }}>{bookTitle}</p>
                </div>
                {bookCoverUrl && (
                  <div className="mt-4">
                    <p className="text-sm font-semibold mb-2 pixel-text" style={{ color: "#6b5210" }}>Book Cover</p>
                    <div className="relative w-full aspect-[2/3] overflow-hidden" style={{ border: "4px solid #8b6914" }}>
                      <Image
                        src={bookCoverUrl}
            ...
            <div className="bg-white/90 backdrop-blur-lg rounded-xl p-6 border-2 border-purple-200 shadow-xl">
              ...
              <Textarea
                ...
                onChange={(e) => {
                  ...
                  trackProcessInput("BOOK_036", e.target.value, { field: "review_edit" }, { stage: "bookReviewEdit" })
                }}
