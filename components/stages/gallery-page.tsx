...
import { BookOpen, Book, Mail, FileText, ChevronDown, ChevronRight, Sparkles, ArrowLeft, MessageSquare } from "lucide-react"
...
function workTypeFromArticleType(t: ArticleType): "story" | "bookReview" | "letter" {
  return t === "bookReview" ? "bookReview" : t === "letter" ? "letter" : "story"
}
    ...
    return (
      <div className={`mt-4 rounded-lg border ${border} ${bg} p-4`}>
        <p className="mb-2 flex items-center gap-2 text-sm font-semibold text-yellow-300">
          ...
          Leave a review (Teacher review or Peer review)
        </p>
        <textarea
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            disabled={!reviewContent[article.id]?.trim() || reviewSubmitting === article.id}
            className="rounded-lg border-indigo-400 text-indigo-200 hover:bg-indigo-900/50"
          >
            Submit as Peer
          </Button>
        </div>
