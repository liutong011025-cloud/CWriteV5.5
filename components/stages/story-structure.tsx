  ...
  const handleSelect = async (structureType: string) => {
    ...
    if (structure) {
      ...
      if (example?.imageUrl) {
        ...
        // 如果示例还没生成，使用占位符或生成一个简单的图片
        imageUrl = `https://api.dicebear.com/7.x/avataaars/svg?seed=${structureType}`
                    ...
                    <div className="grid lg:grid-cols-3 gap-6">
                      {STRUCTURES.map((s, index) => (
                        <div 
                          ...
                          className="bg-gradient-to-br from-white via-purple-50 to-pink-50 rounded-2xl p-8 border-4 border-purple-300 shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 relative overflow-hidden group cursor-pointer"
                        >
                          {/* 装饰背景 */}
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="mt-4 pt-4 border-t-2 border-purple-200">
                              <button className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white py-2 px-4 rounded-lg font-bold text-sm transition-all duration-300 transform hover:scale-105">
