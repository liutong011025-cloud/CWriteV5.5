...
export default function BookSelection({ language = "en", onBookSelected, onBack, userId }: BookSelectionProps) {
  ...
  const handleContinue = async () => {
    ...
    trackProcess("BOOK_006", { book_title: bookTitle }, { stage: "bookSelection" })
  ...
  const handleSend = async () => {
    ...
    // 重置canContinue状态，等待新的AI回复
    setCanContinue(false)
    // 如果是第一次提交或者用户可能换了书名，重置conversationId以确保新对话
    // 注意：API后端会强制使用新对话，但为了前端状态一致性，我们也重置
    const updatedConversation = [...conversation, { role: "user" as const, content: userMessage }]
    setConversation(updatedConversation)
    setBookTitle("")
    trackProcess("BOOK_004", { book_title_text: userMessage }, { stage: "bookSelection" })
    ...
    try {
      ...
      // 更新conversation_id（虽然API每次都是新对话，但保存以便调试）
      if (data.conversationId) {
        setConversationId(data.conversationId)
      } else {
        // 如果没有返回conversationId，设置为null
        setConversationId(null)
      }

      // 确保AI消息不为空才添加到对话
      if (aiMessage.trim()) {
        trackProcess("BOOK_005", { book_title: userMessage, ai_message: aiMessage.slice(0, 4000) }, { stage: "bookSelection" })
        setConversation(prev => {
          const newConversation = [...prev, { role: "assistant" as const, content: aiMessage }]
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" data-no-header style={{ overflowY: 'hidden' }}>
      ...
      <div
        ...
        style={{ paddingTop: 'calc(var(--stage-top-padding) + 18px)', paddingBottom: 'var(--stage-bottom-padding)', overflowY: 'hidden' }}
      >
        {onBack && <BackButton onClick={onBack} variant="blue" />}

        {/* 图书馆场景 */}
        <div className="max-w-7xl mx-auto">
          <div className="relative flex items-center justify-center" style={{ height: 'calc(100vh - 280px)', marginTop: '56px' }}>
            {/* 对话区域 - 两个对话框在同一垂直位置，管理员在上 */}
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
              {/* 等待AI回复时的思考特效 - 大一些，在管理员气泡位置，右上方 */}
              {isLoading && (
                <div className="absolute left-1/2 z-35 w-fit -translate-x-1/2 md:-translate-x-[48%] xl:-translate-x-0 xl:-left-24 top-[-250px] md:top-[-300px] xl:top-[-500px]">
                  <div className="relative">
                    {/* 思考泡泡 - 更大 */}
                    <div className="bg-white/90 rounded-full p-3 md:p-4 shadow-lg border-2 border-amber-300">
              ...
              {(conversation.length === 0 || conversation.filter(msg => msg.role === "assistant").length > 0) && (
                <div className="absolute left-1/2 z-30 w-[min(78vw,18rem)] md:w-[min(72vw,22rem)] xl:w-80 -translate-x-1/2 xl:-translate-x-0 top-[-220px] md:top-[-270px] xl:top-[-470px] xl:-left-32">
                  <div className="bg-amber-100 rounded-2xl p-4 md:p-5 shadow-xl border-2 border-amber-300 relative animate-fade-in">
                    <p className="text-sm md:text-base xl:text-lg text-amber-900 leading-relaxed whitespace-pre-wrap">
              ...
              {conversation.filter(msg => msg.role === "user").length > 0 && (
                <div className="absolute left-1/2 z-20 w-[min(68vw,14rem)] md:w-[min(58vw,16rem)] xl:w-64 -translate-x-1/2 xl:-translate-x-0 top-[-70px] md:top-[-95px] xl:top-[-170px] xl:-left-32">
                  <div className="bg-blue-100 rounded-2xl p-4 md:p-5 shadow-xl border-2 border-blue-300 relative animate-fade-in">
                    <p className="text-sm md:text-base xl:text-lg text-blue-900 leading-relaxed whitespace-pre-wrap">
          ...
          <div className="fixed left-1/2 -translate-x-1/2 z-40 max-w-5xl w-full px-4 md:px-6" style={{ bottom: 'max(24px, var(--stage-bottom-padding))' }}>
            <div className="bg-white/95 backdrop-blur-lg rounded-3xl p-4 md:p-5 xl:p-6 border-2 border-amber-200 shadow-2xl">
              <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-stretch md:items-center">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      trackProcessInput("BOOK_002", e.target.value, { field: "book_title" }, { stage: "bookSelection" })
                    }}
                    onKeyPress={(e) => {
                    ...
                    className="w-full px-4 md:px-6 py-3 md:py-4 rounded-2xl border-2 border-amber-300 focus:border-amber-500 focus:outline-none text-base md:text-lg font-medium bg-gradient-to-r from-amber-50 to-orange-50 focus:from-amber-100 focus:to-orange-100 transition-all duration-300 shadow-inner"
                    disabled={isLoading}
                  />
                </div>
                <div className="flex flex-col sm:flex-row md:flex-col gap-3">
                  <Button
                    ...
                    className="bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white border-0 shadow-xl px-6 md:px-8 py-3 md:py-4 text-base md:text-lg font-bold rounded-2xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                  >
                    {isLoading ? (
                  ...
                  <Button
                    ...
                    } border-0 py-3 md:py-4 px-5 md:px-6 text-sm md:text-base font-bold transition-all duration-300 rounded-2xl whitespace-nowrap`}
                  >
                    {canContinue && conversation.length > 0 && !isLoading ? (
                      <span className="flex items-center gap-2">
                        {t.chooseButton} ✨
