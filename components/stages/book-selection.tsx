...
export default function BookSelection({ language = "en", onBookSelected, onBack, userId }: BookSelectionProps) {
  ...
  return (
    <div className="h-screen relative overflow-hidden" data-no-header style={{ overflowY: 'hidden' }}>
      ...
      {/* 主要内容 */}
      <div
        className="relative z-10 h-screen px-4 md:px-6 xl:px-12 py-8 md:py-10 xl:py-20 pl-4 md:pl-8 xl:pl-20"
        style={{ paddingTop: 'var(--stage-top-padding)', paddingBottom: 'var(--stage-bottom-padding)', overflowY: 'hidden' }}
      >
        {onBack && <BackButton onClick={onBack} variant="blue" />}

        {/* 图书馆场景 */}
        <div className="max-w-7xl mx-auto">
          <div className="relative flex items-center justify-center" style={{ height: 'calc(100vh - 260px)', marginTop: '32px' }}>
            {/* 对话区域 - 两个对话框在同一垂直位置，管理员在上 */}
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
              {/* 等待AI回复时的思考特效 - 大一些，在管理员气泡位置，右上方 */}
              {isLoading && (
                <div className="absolute left-1/2 z-35 w-fit -translate-x-1/2 md:-translate-x-[48%] xl:-translate-x-0 xl:-left-24 top-[-320px] md:top-[-380px] xl:top-[-550px]">
              ...
              {(conversation.length === 0 || conversation.filter(msg => msg.role === "assistant").length > 0) && (
                <div className="absolute left-1/2 z-30 w-[min(78vw,18rem)] md:w-[min(72vw,22rem)] xl:w-80 -translate-x-1/2 xl:-translate-x-0 top-[-290px] md:top-[-350px] xl:top-[-550px] xl:-left-32">
              ...
              {conversation.filter(msg => msg.role === "user").length > 0 && (
                <div className="absolute left-1/2 z-20 w-[min(68vw,14rem)] md:w-[min(58vw,16rem)] xl:w-64 -translate-x-1/2 xl:-translate-x-0 top-[-110px] md:top-[-140px] xl:top-[-200px] xl:-left-32">
