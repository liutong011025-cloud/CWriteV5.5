...
export default function GuidedWriting({ language, storyState, onStoryWrite, onBack, userId, onDraftChange }: GuidedWritingProps) {
  ...
  const [bearPosition, setBearPosition] = useState({ x: 92, y: 8, scale: 1 })
  ...
  useEffect(() => {
    ...
    if (lower.includes("great") || lower.includes("move to the next")) {
      setWritingMood("like")
    } else if (lower.includes("please") || lower.includes("complete")) {
      setWritingMood("sit")
    } else {
      setWritingMood("hang")
    }
                ...
                <div className="relative">
                  <div
                    ...
                    style={{
                      left: `${bearPosition.x}%`,
                      top: `${bearPosition.y}%`,
                      transform: `translate(-50%, -100%) scale(${bearPosition.scale})`,
                    }}
                  >
                    <img
                      src={
                    ...
                    />
                    <div className="mt-1 max-w-xs rounded-2xl border border-purple-300 bg-white/95 px-3 py-2 text-xs text-purple-800 shadow-xl">
                      {isLoadingEvaluation
                        ? "Cagent is thinking..."
            ...
            <div className="bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 rounded-3xl p-6 border-4 border-purple-300 shadow-2xl backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h3 className="text-xl font-bold mb-4 text-purple-700 flex items-center gap-2">
                  ...
                  <span>Bear Bubble Position (Debug)</span>
                </h3>
                <div className="bg-white/85 rounded-xl p-4 border-2 border-purple-200 space-y-2 text-xs">
                  <label className="block">X: {bearPosition.x}%</label>
                  <input type="range" min={50} max={98} value={bearPosition.x} onChange={(e) => setBearPosition((p) => ({ ...p, x: Number(e.target.value) }))} className="w-full" />
                  <label className="block">Y: {bearPosition.y}%</label>
                  <input type="range" min={0} max={25} value={bearPosition.y} onChange={(e) => setBearPosition((p) => ({ ...p, y: Number(e.target.value) }))} className="w-full" />
                  <label className="block">Scale: {bearPosition.scale.toFixed(2)}</label>
                  <input type="range" min={0.7} max={1.4} step={0.05} value={bearPosition.scale} onChange={(e) => setBearPosition((p) => ({ ...p, scale: Number(e.target.value) }))} className="w-full" />
                </div>
              </div>
            </div>
