...
export default function GuidedWriting({ language, storyState, onStoryWrite, onBack, userId, onDraftChange }: GuidedWritingProps) {
  ...
  const handleSectionTextChange = (sectionIndex: number, text: string) => {
    // 檢測到輸入就重置 hang 計時（只有學生什麼也不做才會開始計時）
      ...
      debounceTimerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            console.error("AI evaluation error:", response.status, data)
            setAiEvaluation(msg)
  ...
  return (
    <div className="min-h-screen py-8 px-6 bg-gradient-to-br from-green-100 via-emerald-50 via-blue-50 via-purple-50 to-pink-50 relative overflow-hidden" style={{ paddingTop: '120px', paddingBottom: '120px' }}>
      ...
      <div
        ...
        onClick={() => {
          // 點擊空白區域關閉小熊氣泡（不影響文本框輸入）
          if (!isLoadingEvaluation) {
            setAiEvaluation((prev) => prev)
          }
        }}
      >
            ...
            <div className="bg-gradient-to-br from-white via-blue-50 to-purple-50 rounded-3xl p-8 border-4 border-blue-300 shadow-2xl backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="relative">
                  {/*
                    小熊锚点：定高容器参与 translate(-100%)，气泡绝对定位向下展开，
                    避免气泡撑高布局导致小熊整体上移。
                  */}
                  <div
                  ...
                  >
                    <div className="relative h-28 w-28 md:h-32 md:w-32 shrink-0 overflow-visible">
                      ...
                      {writingMood !== "hang" && showEvaluationBubble && (isLoadingEvaluation || aiEvaluation) && (
                        <div className="absolute left-1/2 top-full z-10 mt-2 w-[min(340px,85vw)] max-h-[min(280px,42vh)] -translate-x-1/2 rounded-2xl border border-purple-300 bg-white/95 px-4 py-3 pt-8 text-sm text-purple-800 shadow-xl">
                          ...
                          <div className="max-h-[min(240px,36vh)] overflow-y-auto whitespace-pre-wrap pr-2 [overflow-anchor:none]">
                            {isLoadingEvaluation
                              ? "Cagent is reading your writing"
                              : aiEvaluation.trim()}
                          </div>
                        </div>
                      )}
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="bg-gradient-to-br from-indigo-50 via-blue-50 to-cyan-50 rounded-3xl p-6 border-4 border-indigo-300 shadow-2xl backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                {/* 不在 UI 中顯示任何 secret/cagent code（避免穿幫）；仍可在內部用於流程控制 */}
              </div>
            </div>
