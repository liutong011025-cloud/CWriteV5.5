      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-6 mt-8">
                  ...
                  >
                    <div className="relative">
                      <button
                        type="button"
                        onClick={() =>
                          setWritingMood((prev) => (prev === "hang" ? "sit" : "hang"))
                        }
                      ...
                      {writingMood === "hang" && isHoveringHang && (
                        <div className="absolute left-1/2 top-full mt-2 -translate-x-1/2 rounded-full bg-black/70 px-4 py-1 text-xs text-white shadow-lg">
                          Save me!
                        </div>
                      )}
                      {writingMood !== "hang" && (isLoadingEvaluation || shortEvaluation) && (
                        <div className="absolute left-1/2 top-full mt-2 w-[320px] -translate-x-1/2 rounded-2xl border border-purple-300 bg-white/95 px-3 py-2 text-xs text-purple-800 shadow-xl">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white/90 rounded-2xl p-4 border-2 border-purple-200 shadow-xl">
              <h3 className="text-sm font-bold text-purple-700 mb-3">Cagent Position Tools</h3>
              <div className="space-y-4 text-xs">
                <div className="rounded-xl border border-blue-200 bg-blue-50/70 p-3">
                  <p className="font-semibold text-blue-700 mb-2">SIT Position Tool</p>
                  <div className="space-y-2">
                    <label className="block">X: {sitPosition.x.toFixed(1)}</label>
                    <input type="range" min="60" max="92" step="0.1" value={sitPosition.x} onChange={(e) => setSitPosition((prev) => ({ ...prev, x: Number(e.target.value) }))} className="w-full" />
                    <label className="block">Y: {sitPosition.y.toFixed(1)}</label>
                    <input type="range" min="-8" max="25" step="0.1" value={sitPosition.y} onChange={(e) => setSitPosition((prev) => ({ ...prev, y: Number(e.target.value) }))} className="w-full" />
                    <label className="block">Scale: {sitPosition.scale.toFixed(2)}</label>
                    <input type="range" min="0.6" max="1.4" step="0.01" value={sitPosition.scale} onChange={(e) => setSitPosition((prev) => ({ ...prev, scale: Number(e.target.value) }))} className="w-full" />
                  </div>
                </div>
                <div className="rounded-xl border border-amber-200 bg-amber-50/70 p-3">
                  <p className="font-semibold text-amber-700 mb-2">HANG Position Tool</p>
                  <div className="space-y-2">
                    <label className="block">X: {hangPosition.x.toFixed(1)}</label>
                    <input type="range" min="60" max="92" step="0.1" value={hangPosition.x} onChange={(e) => setHangPosition((prev) => ({ ...prev, x: Number(e.target.value) }))} className="w-full" />
                    <label className="block">Y: {hangPosition.y.toFixed(1)}</label>
                    <input type="range" min="-8" max="35" step="0.1" value={hangPosition.y} onChange={(e) => setHangPosition((prev) => ({ ...prev, y: Number(e.target.value) }))} className="w-full" />
                    <label className="block">Scale: {hangPosition.scale.toFixed(2)}</label>
                    <input type="range" min="0.6" max="1.4" step="0.01" value={hangPosition.scale} onChange={(e) => setHangPosition((prev) => ({ ...prev, scale: Number(e.target.value) }))} className="w-full" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-indigo-50 via-blue-50 to-cyan-50 rounded-3xl p-6 border-4 border-indigo-300 shadow-2xl backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                {goodEnoughSecret && (
                  <div className="mt-3 rounded-xl border-2 border-emerald-300 bg-emerald-50 px-4 py-2 text-center text-sm font-semibold text-emerald-700">
                    Secret code unlocked: {goodEnoughSecret}
                  </div>
                )}
              </div>
            </div>
