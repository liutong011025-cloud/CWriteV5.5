...
import WritingFeedbackDialog from "@/components/feedback/writing-feedback-dialog"
...
export default function LetterEdit({
  ...
  // 第一次進入編輯頁時，如果學生從未給過反饋，彈出一次體驗調查
  useEffect(() => {
    if (!userId || typeof window === "undefined") return
    const key = `writingFeedbackShown_${userId}`
  ...
  const handleFeedbackClosed = () => {
    if (userId && typeof window !== "undefined") {
      localStorage.setItem(`writingFeedbackShown_${userId}`, "1")
    }
