...
export default function PlotBrainstorm({ language, character, onPlotCreate, onBack, userId }: PlotBrainstormProps) {
  ...
  const falPlotIterationSentRef = useRef(false)
...
- Do NOT output Chinese.`
  ...
  // When plot is ready (right before structure selection), send Plot Summary to Fal for iteration
  // to replace any older "plot progress" style uploads.
  useEffect(() => {
    if (!canProceed) return
    if (falPlotIterationSentRef.current) return
    falPlotIterationSentRef.current = true

    const prompt = [
      "Children's story concept art, warm pixel-fantasy style.",
      `Setting: ${plotData.setting}`,
      `Problem: ${plotData.conflict}`,
      `Goal: ${plotData.goal}`,
    ].join("\n")
  ...
  }, [canProceed, plotData.setting, plotData.conflict, plotData.goal, userId])
