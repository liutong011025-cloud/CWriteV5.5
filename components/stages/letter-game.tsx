            ...
            <div className="bg-white/90 backdrop-blur-lg rounded-2xl p-6 border-4 border-pink-300 shadow-xl relative">
              ...
              {showBearTool && (
                <div className="absolute right-3 top-11 z-30 w-52 rounded-xl border border-purple-200 bg-white/95 p-2 text-xs shadow-lg">
                  ...
                  <input type="range" min={10} max={98} step={0.1} value={bearPosition.x} onChange={(e) => setBearPosition((p) => ({ ...p, x: Number(e.target.value) }))} className="w-full" />
                  <label className="mb-1 mt-2 block">Y: {bearPosition.y.toFixed(1)}%</label>
                  <input type="range" min={-20} max={80} step={0.1} value={bearPosition.y} onChange={(e) => setBearPosition((p) => ({ ...p, y: Number(e.target.value) }))} className="w-full" />
                  <label className="mb-1 mt-2 block">缩放: {bearPosition.scale.toFixed(2)}</label>
                  <input type="range" min={0.4} max={2.2} step={0.01} value={bearPosition.scale} onChange={(e) => setBearPosition((p) => ({ ...p, scale: Number(e.target.value) }))} className="w-full" />
                </div>
              )}
              <div
