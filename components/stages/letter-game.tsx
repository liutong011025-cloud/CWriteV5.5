...
export default function LetterGame({
  ...
  const activeBearPosition = {
    x: LETTER_BEAR_LAYOUT.bearX,
    y: writingMood === "hang" ? LETTER_BEAR_LAYOUT.hangY : LETTER_BEAR_LAYOUT.bearY,
    scale: LETTER_BEAR_LAYOUT.bearScale,
  }
  ...
  const handleTextChange = (text: string) => {
    ...
    trackProcessInput("LETTER_012", text, { section_index: currentSection }, { stage: "letterGame" })
  }
  ...
  const handleNext = () => {
    ...
    if (currentSection < LETTER_SECTIONS.length - 1) {
      trackProcess("LETTER_018", { from_section: currentSection, to_section: currentSection + 1 }, { stage: "letterGame" })
  ...
  const handlePrevious = () => {
    if (currentSection > 0) {
      trackProcess("LETTER_017", { from_section: currentSection, to_section: currentSection - 1 }, { stage: "letterGame" })
      setCurrentSection(prev => prev - 1)
  ...
  const handleFinish = () => {
    trackProcess("LETTER_021", { section_count: LETTER_SECTIONS.length }, { stage: "letterGame" })
    // 测试模式下直接完成
    if (isTestMode) {
