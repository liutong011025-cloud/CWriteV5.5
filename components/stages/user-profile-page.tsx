...
export default function UserProfilePage({
  ...
  const [farmElementStates, setFarmElementStates] = useState<Record<FarmElementId, FarmElementState>>({
    farmbacktomap: isOtherFarm ? { x: 72.5, y: 43.2, scale: 0.7 } : { x: 74.1, y: 43.7, scale: 0.7 },
    farmsetting: isOtherFarm ? { x: 38.0, y: 47.8, scale: 0.8 } : { x: 34.8, y: 50.0, scale: 0.8 },
                          ...
                          {items.length === 0 ? (
                            ...
                            <ul className="space-y-2">
                              {items.map((w) => (
                                <li
                                  key={w.id}
                                  className="flex items-center gap-3 rounded-xl border border-amber-100 bg-amber-50/50 px-3 py-2 font-hand"
                                >
    ...
    >
      <div className="w-full h-screen">
        ...
        <div ref={farmContainerRef} className="relative w-full h-full overflow-hidden">
          ...
          {isOtherFarm && (
            <div className="absolute right-4 top-4 z-[60] w-[320px] rounded-2xl border border-purple-200 bg-white/90 p-3 shadow-xl backdrop-blur-sm">
              <h4 className="mb-2 font-hand text-sm font-bold text-purple-700">Other Farm Position Debug</h4>
              {[
                { id: "farmbacktomap" as const, label: "Back to Map" },
                { id: "farmsetting" as const, label: "Their Map" },
                { id: "farmwrittingboard" as const, label: "Writing Board" },
              ].map((item) => {
                const state = farmElementStates[item.id]
                return (
                  <div key={item.id} className="mb-3 rounded-lg border border-purple-100 bg-purple-50/60 p-2">
                    <div className="mb-1 text-xs font-semibold text-purple-700">{item.label}</div>
                    <div className="text-[11px] text-gray-600">X: {state.x.toFixed(1)}%</div>
                    <input
                      type="range"
                      min={0}
                      max={100}
                      ...
                      className="w-full"
                    />
                    <div className="text-[11px] text-gray-600">Y: {state.y.toFixed(1)}%</div>
                    <input
                      type="range"
                      min={0}
                      max={100}
                      ...
                      className="w-full"
                    />
                    <div className="text-[11px] text-gray-600">Scale: {state.scale.toFixed(2)}</div>
                    <input
                      type="range"
                      min={0.2}
                      max={1.6}
                      ...
                      onChange={(e) => {
                        ...
                        setFarmElementStates((prev) => ({
                          ...prev,
                          [item.id]: { ...prev[item.id], scale: value },
                        }))
                      }}
                      className="w-full"
                    />
          ...
          {isOtherFarm && showOtherMapDialog && (
            ...
            >
              ...
              >
                ...
                <img
                  src={otherMapImageUrl || "/theirmap.png"}
                  alt={`${userId} map`}
