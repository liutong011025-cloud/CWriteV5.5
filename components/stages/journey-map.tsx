...
const buildDramaTasks = (dramaState?: any): MapTask[] => {
  const hasBackground = !!dramaState?.background?.imageUrl
  const hasScenes = dramaState?.scenes && dramaState.scenes.length > 0
  const hasComplete = hasBackground && hasScenes

  // 移除welcome任务点，只保留writing和complete
  const taskCount = 2
  return [
    {
      id: "drama-writing",
    ...
    },
    {
      ...
      y: 48,
      isComplete: hasComplete,
      isEnabled: hasComplete,
      title: "Drama Complete",
    },
  ]
}
...
export default function JourneyMap({
  ...
  const getTasks = (): MapTask[] => {
    ...
    if (type === "drama") return buildDramaTasks(dramaState)
