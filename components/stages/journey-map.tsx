...
const buildBookReviewTasks = (bookReviewState: BookReviewState, noAi?: boolean): MapTask[] => {
  ...
  const tasks: MapTask[] = [
        ...
        ]),
    {
      id: "review-writing",
      stage: writingStage,
      // 中間寫作節點：noAi 時是第二個，AI 時是第三個
      x: (100 / (taskCount + 1)) * (noAi ? 2 : 3),
      y: 48, // 略向下移动，靠近中线
      isComplete: hasReview,
      isEnabled: hasBook && hasType && (noAi ? true : hasStructure),
      title: "Writing",
    },
    {
      id: "review-complete",
...
const buildDramaTasks = (dramaProgress?: DramaProgress): MapTask[] => {
  const hasDramaBook = !!dramaProgress?.hasDramaBook
  const taskCount = 2
  return [
    {
      id: "drama-builder",
    ...
    },
    {
      ...
      y: 48,
      isComplete: hasDramaBook,
      isEnabled: hasDramaBook,
      title: "Drama Book",
    },
  ]
}

const buildPoetryTasks = (poetryProgress?: PoetryProgress): MapTask[] => {
  const hasForm = !!poetryProgress?.hasForm
  const hasTopic = !!poetryProgress?.hasTopic
  const hasLines = !!poetryProgress?.hasLines
  const phase = poetryProgress?.phase ?? "choose-form"
  const taskCount = 4
  return [
    {
      id: "poetry-form",
...
export default function JourneyMap({
  ...
  const getTasks = (): MapTask[] => {
    ...
    if (type === "drama") return buildDramaTasks(dramaProgress)
    if (type === "poetry") return buildPoetryTasks(poetryProgress)
  ...
  const handleSelect = (task: MapTask) => {
    if (!task.isEnabled || !task.stage) {
      toast.info("Complete the previous task first to unlock this one.")
      return
    }
    if (task.isComplete) {
