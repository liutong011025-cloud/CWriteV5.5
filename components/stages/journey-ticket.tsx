...
export default function JourneyTicket({
  ...
  return (
    <div className="min-h-screen relative overflow-hidden pixel-theme">
      ...
      <div className="relative z-10 min-h-screen px-6 lg:px-14 py-14 lg:py-24 pl-16 lg:pl-24" style={{ paddingTop: "128px", paddingBottom: "120px" }}>
        {onBack && <BackButton onClick={onBack} variant="purple" aria-label="Back to Map" />}

        {/* Pixel-style title */}
        <div className="text-center mb-12">
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-black mb-5 pixel-title" style={{
            color: "#8b6914",
            textShadow: "3px 3px 0 #6b5210, 5px 5px 0 rgba(0,0,0,0.2)"
          }}>
            Your Journey Ticket
          </h1>
          <p className="text-xl md:text-2xl pixel-text" style={{ color: "#5a4a2a" }}>
            Drag a journey type onto the ticket, then choose your difficulty.
          </p>
        </div>

        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-10 mb-12">
          {/* Left side - Journey type cards with pixel styling */}
          <div className="flex flex-col justify-center lg:justify-start">
            <p className="text-base mb-5 text-center lg:text-left max-w-[320px] pixel-text" style={{ color: "#5a4a2a" }}>
              ...
              {journeyOptions.map((option, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`relative p-5 cursor-pointer transition-all duration-200 pixel-btn bg-gradient-to-b ${option.pixelBg}`}
                    style={{
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="text-4xl" style={{ imageRendering: "pixelated" }}>{option.icon}</span>
                      <h3 className="text-xl font-extrabold text-white pixel-text">
                        {option.title}
                      </h3>
                    </div>
                  </div>
          ...
          <div className="max-w-5xl mx-auto">
            <div className={`relative pixel-panel p-10 ${isDeparting ? "animate-ticket-tear" : ""}`}>
              {/* Stamp effect */}
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-3xl font-extrabold" style={{ color: "#8b6914" }}>Level {difficulty}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-3 text-xs uppercase tracking-widest px-4 py-1 pixel-chip" style={{ color: "#8b6914" }}>
                BOARDING PASS
              </div>
              ...
              {/* Pixel airplane */}
              <div className={`absolute -top-7 right-10 text-5xl ${isDeparting ? "plane-flyaway" : ""}`} style={{ imageRendering: "pixelated" }}>
                ✈️
              </div>
              ...
              <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-6">
                <div>
                  ...
                  <div className="grid md:grid-cols-3 gap-6 items-center">
                    <div className="space-y-2">
                      <p className="text-xs uppercase tracking-widest pixel-text" style={{ color: "#8b6914" }}>Passenger</p>
                      <p className="text-2xl font-bold pixel-text" style={{ color: "#5a4a2a" }}>{userName}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs uppercase tracking-widest pixel-text" style={{ color: "#8b6914" }}>Writing Level</p>
                      <p className="text-2xl font-bold pixel-text" style={{ color: "#7ec850" }}>Level {level}</p>
                      <div className="flex items-center gap-3">
                        <p className="text-sm pixel-text" style={{ color: "#6b5210" }}>Score: {score}/7</p>
                        {onRetest && (
                          <button
                            type="button"
                            className="px-3 py-1 text-xs font-bold text-white pixel-btn pixel-btn-blue"
                            onClick={onRetest}
                          >
                            Retest
                          </button>
                    ...
                    <div className="space-y-2">
                      <p className="text-xs uppercase tracking-widest pixel-text" style={{ color: "#8b6914" }}>Journey</p>
                      <p className="text-2xl font-bold pixel-text" style={{ color: "#5a4a2a" }}>
                  ...
                  <div className="mt-6 grid sm:grid-cols-3 gap-4 text-sm">
                    <div className="pixel-card p-3">
                      ...
                      <p className="text-lg font-bold" style={{ color: "#5a4a2a" }}>YL-2024</p>
                    </div>
                    <div className="pixel-card p-3">
                      <p className="text-[11px] uppercase tracking-widest" style={{ color: "#8b6914" }}>Gate</p>
                      <p className="text-lg font-bold" style={{ color: "#5a4a2a" }}>LUNA</p>
                    </div>
                    <div className="pixel-card p-3">
                      <p className="text-[11px] uppercase tracking-widest" style={{ color: "#8b6914" }}>Seat</p>
                      <p className="text-lg font-bold" style={{ color: "#5a4a2a" }}>A1</p>
                    </div>
                  </div>

                  {/* Difficulty selector with pixel buttons */}
                  <div className="mt-8">
                    <p className="text-sm uppercase tracking-widest mb-3 pixel-text" style={{ color: "#8b6914" }}>Choose Difficulty</p>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((value) => (
                        <button
                          key={value}
                          onClick={() => setDifficulty(value)}
                          className={`px-6 py-2 font-bold text-sm transition-all pixel-btn ${
                            difficulty === value
                    ...
                    {notice && <p className="mt-3 text-sm font-semibold pixel-text" style={{ color: "#c9a82e" }}>{notice}</p>}
                  </div>
                </div>
                ...
                <div className="relative flex flex-col items-center justify-between">
                  {/* Pixel barcode stub */}
                  <div className="w-full max-w-[240px] pixel-card p-4 text-center">
                    <p className="text-xs uppercase tracking-widest mb-3" style={{ color: "#8b6914" }}>Boarding</p>
                    <div className="h-[52px] rounded" style={{
                  ...
                  <div className="mt-6 flex flex-col items-center">
                    <p className="text-xs uppercase tracking-widest mb-3 pixel-text" style={{ color: "#8b6914" }}>Drop Type</p>
                    <div
                      className={`w-24 h-24 flex items-center justify-center text-center text-xs font-bold transition-all ${
                        dragOver || isDragging
                          ? "pixel-selected"
                          : selectedType
                            ? ""
        ...
        <div className="text-center mt-10 space-y-3">
          <Button
            ...
            className="pixel-btn pixel-btn-green text-xl md:text-2xl font-bold py-6 px-14 disabled:opacity-50 disabled:cursor-not-allowed"
            style={{
              borderRadius: 0,
