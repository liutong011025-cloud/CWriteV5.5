  ...
  return (
    <div className={`relative z-40 ${className}`} ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 rounded-xl border-2 border-amber-400/90 bg-gradient-to-r from-amber-100 to-yellow-100 px-3 py-1.5 font-bold text-amber-900 shadow-lg transition hover:from-amber-200 hover:to-yellow-200 focus:outline-none focus:ring-2 focus:ring-amber-500"
        aria-label={`Level ${level} - ${detail.subtitle}`}
      >
        <span className="text-sm">Lv.{level}</span>
        <span className="text-xs opacity-90">{(detail.subtitle || "").split("—")[0]?.trim() || detail.title}</span>
      </button>
      {open && (
        <div className="absolute left-0 top-full mt-2 w-[min(90vw,400px)] rounded-2xl border-2 border-amber-200 bg-white p-4 shadow-2xl">
