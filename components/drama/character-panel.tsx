...
function PresetCharacterCell({
  ...
  return (
    <button
      ...
      onClick={onPick}
      className={cn(
        "group relative aspect-square w-full overflow-hidden rounded-xl border-2 border-[#4a2a15] bg-[#7a4a1f] p-0 transition-all",
        "hover:z-10 hover:-translate-y-0.5 hover:border-[#6b3e1e] active:scale-95",
      )}
      title={preset.species}
    >
      <div className="absolute inset-0 flex items-center justify-center p-0.5 pb-5">
        {failed ? (
          ...
          <img
            src={preset.imageUrl}
            alt={preset.species}
            className="max-h-full max-w-full scale-[1.12] object-contain drop-shadow-[0_3px_12px_rgba(0,0,0,0.28)]"
      ...
      <span className="pointer-events-none absolute bottom-0 left-0 right-0 truncate bg-gradient-to-t from-black/55 via-black/25 to-transparent px-1 pb-0.5 pt-3 text-center font-hand text-[9px] font-bold leading-tight text-[#f7e6d0]">
        {preset.species}
      </span>
    </button>
  );
    ...
    <div className="styled-scrollbar sticky top-28 flex max-h-[calc(100vh-8rem)] flex-col gap-4 overflow-y-auto rounded-2xl border border-slate-200 bg-gradient-to-br from-amber-50/70 via-white to-sky-50/60 p-4 shadow-sm">
      ...
      <Dialog open={toyBoxOpen} onOpenChange={handleToyBoxOpenChange}>
        <DialogContent
          ...
          className="max-h-[92vh] overflow-y-auto sm:max-w-[min(74rem,calc(100%-2rem))] bg-gradient-to-br from-amber-50 via-white to-sky-50"
        >
          {toyBoxStep === "pick" ? (
            <>
              <DialogHeader>
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="font-hand text-[17px] leading-relaxed">
                  Type the character name. Species/type comes from the image file name.
                </DialogDescription>
              </DialogHeader>
              <div className="flex flex-col items-center gap-4">
                {pickedPreset && (
                  <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-3">
                    ...
                    />
                    <p className="mt-2 w-full truncate px-1 text-center font-hand text-[12px] font-bold text-muted-foreground">
