...
function PresetCharacterCell({
  ...
  return (
    <button
      ...
      onClick={onPick}
      className={cn(
        "group relative aspect-square overflow-hidden rounded-xl border-2 border-slate-200 bg-white shadow-sm transition-all",
        "hover:z-10 hover:border-primary hover:shadow-md active:scale-95",
      )}
      title={preset.species}
    >
      {failed ? (
        <span className="flex h-full w-full items-center justify-center px-0.5 text-center font-hand text-[9px] leading-tight text-muted-foreground">
          {preset.species}
        </span>
      ) : (
  ...
  return (
    <div className="styled-scrollbar sticky top-28 flex max-h-[calc(100vh-8rem)] flex-col gap-4 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <Dialog open={toyBoxOpen} onOpenChange={handleToyBoxOpenChange}>
        <DialogContent
          showCloseButton
          className="max-h-[90vh] overflow-y-auto sm:max-w-[min(56rem,calc(100%-2rem))] bg-white"
        >
          {toyBoxStep === "pick" ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription className="font-hand text-sm">
                  Tap a doll to choose its look. You will name them on the next
                  step. You can still use &quot;Generate Look!&quot; later for
                  AI images.
                </DialogDescription>
              </DialogHeader>
              <div
                className="rounded-2xl border-2 border-amber-200/70 bg-gradient-to-b from-amber-50/90 to-white p-3 shadow-inner"
                style={{
                  boxShadow: "inset 0 2px 0 rgba(255,255,255,0.8)",
                }}
              >
                <div className="mb-2 flex items-center justify-center gap-2 border-b border-amber-200/50 pb-2">
                  <div className="h-2 w-16 rounded-full bg-amber-300/80" />
                </div>
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="font-hand text-sm">
                  Name is yours to choose. Species/type for the script is taken
                  from the image file:{" "}
