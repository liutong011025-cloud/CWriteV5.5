...
function PresetCharacterCell({
  ...
  onPick: () => void;
  onHoverStart?: (preset: DramaPresetCharacter) => void;
  onHoverEnd?: () => void;
  isHovered?: boolean;
  key?: string;
}) {
      ...
      <div className="absolute inset-0 flex items-center justify-center p-1">
        {failed ? (
          ...
          <img
            src={preset.imageUrl}
            alt={preset.species}
            className="max-h-full max-w-full scale-[1.10] object-contain drop-shadow-[0_8px_18px_rgba(0,0,0,0.42)] transition-transform"
...
export function CharacterPanel() {
  ...
  const [hoveredPreset, setHoveredPreset] =
    useState<DramaPresetCharacter | null>(null);
          ...
          {toyBoxStep === "pick" ? (
            <>
              ...
              >
                ...
                {hoveredPreset && (
                  <div className="pointer-events-none absolute left-1/2 top-1/2 z-50 w-40 -translate-x-1/2 -translate-y-1/2">
                    <img
                      src={hoveredPreset.imageUrl}
                      alt={hoveredPreset.species}
                      draggable={false}
                      className="h-[250px] w-full object-contain drop-shadow-[0_18px_40px_rgba(0,0,0,0.55)] transition-transform duration-150 rotate-[-3deg]"
                    />
                  </div>
                )}
                  ...
                  <div className="relative grid grid-cols-5 gap-1.5">
                    {DRAMA_PRESET_CHARACTERS.map((preset) => (
                      <PresetCharacterCell
                        ...
                        isHovered={hoveredPreset?.imageUrl === preset.imageUrl}
                      />
                    ))}
                  </div>
                </div>
