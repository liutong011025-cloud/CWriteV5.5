...
function PresetCharacterCell({
  ...
  return (
    <button
      ...
      onClick={onPick}
      className={cn(
        "group relative flex aspect-square w-full flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-[#4a2a15] bg-[#7a4a1f] px-1 py-1 shadow-[inset_0_2px_0_rgba(255,255,255,0.18)] transition-all",
        "hover:z-10 hover:-translate-y-0.5 hover:border-[#6b3e1e] hover:shadow-[inset_0_2px_0_rgba(255,255,255,0.25),0_12px_22px_rgba(0,0,0,0.18)] active:scale-95",
      )}
      title={preset.species}
    >
      <div className="flex h-[58%] w-full items-center justify-center">
        {failed ? (
          <span className="font-hand text-[14px] font-bold text-[#f7e6d0]">?</span>
        ) : (
      ...
      <span className="mt-1 w-full truncate px-1 text-center font-hand text-[9px] font-bold leading-tight text-[#f7e6d0]">
        {preset.species}
      </span>
    </button>
  );
...
export function CharacterPanel() {
  ...
  return (
    <div className="styled-scrollbar sticky top-28 flex max-h-[calc(100vh-8rem)] flex-col gap-4 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <Dialog open={toyBoxOpen} onOpenChange={handleToyBoxOpenChange}>
        <DialogContent
          showCloseButton
          className="max-h-[90vh] overflow-y-auto sm:max-w-[min(56rem,calc(100%-2rem))] bg-gradient-to-br from-amber-50 via-white to-sky-50"
        >
          {toyBoxStep === "pick" ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription className="font-hand text-sm">
                  Choose one character doll. You will name them on the next
                  step. Species/type is taken from the image file name.
                </DialogDescription>
              </DialogHeader>
              <div
                className="rounded-2xl border-4 border-[#4a2a15] bg-[#6b3e1e] p-4 shadow-inner"
                style={{
                  backgroundImage:
                    "repeating-linear-gradient(90deg, rgba(255,255,255,0.07) 0, rgba(255,255,255,0.07) 2px, transparent 2px, transparent 26px), repeating-linear-gradient(0deg, rgba(0,0,0,0.12) 0, rgba(0,0,0,0.12) 2px, transparent 2px, transparent 26px)",
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="rounded-full bg-[#8b5a2b]/70 px-2 py-0.5 font-hand text-[10px] font-bold text-[#f7e6d0]">
                    40 slots
                  </div>
                </div>
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="font-hand text-sm">
                  Species/type for the script is taken from the image file:{" "}
                  <span className="font-semibold text-foreground">
                    {pickedPreset?.species}
                  </span>
                  .
                </DialogDescription>
              </DialogHeader>
              <div className="flex flex-col items-center gap-4">
                {pickedPreset && (
                  <div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-3">
                    ...
                    />
                    <p className="mt-2 w-full truncate px-1 text-center font-hand text-[11px] font-bold text-muted-foreground">
                      {pickedPreset.species}
                    </p>
                  </div>
                )}
