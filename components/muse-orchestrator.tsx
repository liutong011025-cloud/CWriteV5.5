...
type MuseOrchestratorProps = {
  mode?: "full" | "writing"
  initialStage?: OrchestratorStage
}

export default function MuseOrchestrator({
  mode = "writing",
  initialStage = "writeTypeSelection",
}: MuseOrchestratorProps) {
  const router = useRouter()
  const [user, setUser] = useState<AppUser | null>(null)
  const [stage, setStage] = useState<OrchestratorStage>(initialStage)
  ...
  const goMyFarm = useCallback(() => router.push("/my-farm"), [router])
  const goLibrary = useCallback(() => router.push("/library"), [router])
  const goWritingHub = useCallback(() => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      // 嘗試恢復已登入用戶；writing 模式无用户则回登录页
      try {
        const parsed = getStoredUser() as AppUser | null
        if (parsed) {
          setUser(parsed)
  ...
  }, [user, mode, router, goMyFarm, goLibrary])
