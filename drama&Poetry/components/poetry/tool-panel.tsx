  ...
  const fetchRhymes = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/dify", {
        ...
        body: JSON.stringify({ action: "rhyme", topic, word }),
      });
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/dify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "syllable", topic, word: line.text, lineIndex: activeLineIndex }),
      });
      const data = await res.json();
      ...
      {suggestions.length > 0 && (
        <div className="space-y-1.5">
          {suggestions.map((s, i) => {
            ...
            // Category header (e.g. "Shorter:" or "Longer:") -- no Use button
            if (headerMatch) {
              return (
            ...
            // Actual suggestion line -- show Use button
            return (
              <div key={i} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-card px-3 py-2">
...
function FreeVersePanel({ topic, activeLineIndex, onInsertText, onReplaceLine }: { topic: string; activeLineIndex: number; onInsertText: (text: string) => void; onReplaceLine: (text: string) => void }) {
  ...
  const activeLine = lines[activeLineIndex]?.text || "";
          ...
          {activeLine.trim() && (
            <div className="rounded-lg bg-muted/50 px-2.5 py-1.5">
              <p className="font-hand text-[10px] text-muted-foreground">
                Your line: <span className="font-bold text-foreground">{activeLine}</span>
              </p>
            </div>
          )}
          {examples.map((ex, i) => {
