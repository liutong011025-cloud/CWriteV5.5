...
// Fetcher function for SWR - extracts interactions array from response
...
// Extract articles from interactions data (matches API response format)
function extractArticlesFromInteractions(interactions: any[]): Article[] {
  const articles: Article[] = []

  for (const i of interactions) {
    // Stories
    if (i.story && (i.stage === 'review' || (typeof i.story === 'string' && i.story.trim().length > 0))) {
    ...
    // Letters
    if (i.letter && (i.stage === 'letterComplete' || (typeof i.letter === 'string' && i.letter.trim().length > 0))) {
...
// Example articles for when database is empty
...
// Custom hook to use gallery data with SWR caching
export function useGalleryData() {
  const { data: interactions, error, isLoading, mutate } = useSWR(
    "/api/interactions",
    fetcher,
    {
  ...
  const articles = interactions 
    ? [...extractArticlesFromInteractions(interactions), ...EXAMPLE_ARTICLES].sort((a, b) => b.timestamp - a.timestamp)
    : EXAMPLE_ARTICLES
...
// Preload function to call from layout/page
export function preloadGalleryData() {
  // Trigger the fetch early - SWR will cache it
  fetch("/api/interactions").catch(() => {})
}
