...
Please evaluate the student's writing for the current section. In 5-7 SHORT sentences:
- Sentence 1: mention one concrete detail from the student's text (for example a word, action, or idea) and praise it, using words that match their level (very simple for Level 1–2, a bit richer for Level 3–5).
- Sentence 2-3: give 1-2 clear next-step suggestions for THIS section (tell the child exactly what to add or change in the next 1–2 sentences).
- Sentence 4: quickly check grammar/spelling. If there are mistakes, quote the wrong part in quotes and then write the correct version, like: "He go to school" → "He goes to school". If everything is clear enough for this level, just say it is clear.
- Sentence 5: mention what positive values are shown (for example kindness, responsibility, respect), or say which value is missing and how they can show it.
- Sentence 6: connect your feedback to the student's level ${level} (for example: "For your level, this is already strong because..." or "At your level, try to…").
- If writing is not good enough, clearly tell the child how to revise and improve before passing (at least one very concrete fix).
...
- If and only if you wrote "done", add one more NEW LINE with this exact secret code: "${GOOD_ENOUGH_CODE}" and also include this exact English sentence somewhere in your feedback: "You can move on to the next part of your writing!" (do not translate this sentence).
      ...
      return NextResponse.json(
        {
          error: 'empty_answer',
          message:
            'The writing coach did not return any text. Check Dify app logs, model timeout, or that this API key matches the Writing Evaluation app.',
        },
        { status: 502 }
      )
    }
