...
1. Setting: Extract location/setting. Can be a single word or short phrase.
    2. Conflict: Extract the problem/challenge as a SHORT PHRASE (1-6 words). Students may answer with a single word; if it is a clear problem/challenge word, accept it as-is. Avoid phrases like "is lost" without a clear object (if the object is missing, use the given problem word).
       Examples: "lost map", "can't find the map", "dark", "hungry", "monster".
    3. Goal: Extract what the character wants as a SHORT PHRASE (1-6 words). Students may answer with a single word; if it is a clear want/action word, accept it as-is. Example: "find the library", "escape", "friend", "brave", "owner".

IMPORTANT:
    - Conflict and Goal MUST be coherent short phrases (1-6 words), not just a label
- If student hasn't discussed enough, use "unknown"
- Be generous - extract even if brief; if the student only gave one problem/goal word, use it
