...
- Evidence rules are strict:
  - Only include a dimension if the writing has direct textual evidence.
  - "sentence" must be quoted or copied from the student's writing.
  - "reason" must explain why that sentence supports that specific value.
  - If evidence is weak or unclear, DO NOT include that dimension.
`.trim()
...
`.trim()
    ...
    if (rawEvidence && typeof rawEvidence === "object") {
      Object.entries(rawEvidence as Record<string, unknown>).forEach(([k, v]) => {
        ...
        if (!sentence || !reason) return
        evidenceByDimension[id] = { sentence, reason }
      })
    }
