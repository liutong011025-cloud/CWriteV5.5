...
[
  ...
  },
  {
    ...
    "original": "go",
    "corrected": "goes",
    "issue": "Subject-verb agreement: 'go' should be 'goes' with third person singular subject"
  },
  {
    "start": 0,
    "end": 0,
    "original": "tresure",
    "corrected": "treasure",
    "issue": "Spelling error: 'tresure' should be spelled 'treasure'"
  }
]`
    ...
    const callDify = async () => {
      ...
      if (!response.ok) {
        ...
        return { ok: false as const, status: response.status, data: null as Record<string, unknown> | null }
      }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (!result.ok || !result.data) {
        ...
        return NextResponse.json({
          success: true,
          errors: buildLocalGrammarErrors(content),
          source: `fallback_local_http_${result.status}`,
        })
      }
      data = result.data
      aiResponse = String(data.answer || data.message || '').trim()
      if (aiResponse) break
    ...
    const errorCountMap = new Map<string, number>() // 跟踪每个错误单词已处理的数量
    ...
    validErrors.forEach((error: any) => {
      ...
      if (matches.length === 0) {
        ...
        if (fuzzyMatches.length > 0) {
          ...
          // 如果匹配数量少于错误索引，说明没有足够的匹配位置，跳过
          if (errorIndex >= fuzzyMatches.length) {
            return
          }
