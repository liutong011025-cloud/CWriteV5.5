...
export async function POST(request: NextRequest) {
  try {
    ...
    // 构建查询消息：只判断这本书是否适合小学生寫書評
    const queryMessage = `You are a judge evaluating whether a book is suitable for an elementary student to write a book review about. Speak like a friendly, cute elementary teacher with a warm and encouraging tone. Use emojis appropriately. Keep your response to about 30-40 words (a bit longer to be more friendly and cute).
...
Be warm, cute, and encouraging. Use English.${levelSuffix}`
    ...
    const bookSelectionUserId = `book-selection-${user_id || 'default-user'}-${timestamp}`
