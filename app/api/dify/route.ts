...
import { getLevelInstruction, getLevelPromptSuffix } from "@/lib/level-details";

const DIFY_API_URL = "https://api.dify.ai/v1/chat-messages";
const DIFY_API_KEY = "app-TFDykrjN8LpJROY6eTRNjwo5";

/**
 * Unified Dify AI endpoint for Poetry (rhyme, syllable, rhetoric, synonym, review, poetry_revise).
 * Drama actions (drama_generate, drama_revise) are also supported for compatibility.
 * Supports level (1-5) for level-appropriate prompts.
 */
  ...
  try {
    ...
    switch (action) {
      case "book_review_type": {
        ...
        prompt = `You are a helpful book review coach for elementary school students (ages 8-12).

The student has chosen the book: "${bookTitle}".
...
Give brief, encouraging feedback (2-3 sentences) about what they changed and how it improved. Be specific about what got better. Use simple, friendly language.${levelSuffix}`;
    ...
    if (!response.ok) {
      ...
      console.error("[dify] Dify error:", response.status, errText);
    ...
    if (action === "book_review_type") {
      try {
        ...
        console.error("[dify] Failed to parse book_review_type JSON:", e, answer);
        return NextResponse.json({
    ...
    console.error("[dify] Dify error:", error);
