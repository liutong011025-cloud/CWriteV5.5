...
export async function POST(request: NextRequest) {
  try {
    const { text, reviewType, bookTitle, structure, currentSection, user_id, level: levelRaw } = await request.json()
    ...
    // 构建查询消息 - 降低通过标准
    const queryMessage = `You are Luna, a friendly book review writing teacher for elementary students.
...
Remember: Be encouraging and supportive. Approve reasonable writing that shows effort, even if brief. Only say "you can move to the next part" when the writing is good enough for its purpose or his total number of words is beyond 10.如果有乱码组成或者显然没有逻辑，一定不要说you can move to the next part这句话${levelSuffix}`
    ...
    const message = extractDifyAnswer(data)
    if (!message) {
      console.error('Dify book writing aid: empty answer', JSON.stringify(data).slice(0, 2000))
      return NextResponse.json(
        {
          error: 'empty_answer',
          message: 'No text returned from Dify. Check app type (Chat vs Workflow) and API key.',
        },
        { status: 502 }
      )
    }
    ...
    if (aborted) {
      return NextResponse.json(
        {
          error: 'timeout',
          message: 'Dify took too long. Try a shorter paragraph or check model settings.',
        },
        { status: 504 }
      )
    }
