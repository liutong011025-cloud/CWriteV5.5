...
function isMissingTableError(e: unknown): boolean {
  ...
  return (
    code === "P2021" ||
    /relation ["']?(work_reviews)["']? does not exist/i.test(msg) ||
    /table ["']?work_reviews["']? does not exist/i.test(msg)
  )
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isMissingTableError(e)) {
      return NextResponse.json({ reviews: [], unreadCount: 0 })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (missingTable) {
      return NextResponse.json(
        { error: "Reviews not available. Please run: npx prisma migrate deploy" },
        { status: 503 }
      )
    }
    return NextResponse.json({ error: "Internal error" }, { status: 500 })
