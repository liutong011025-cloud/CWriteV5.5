  ...
  return (
    <main className="min-h-screen">
      ...
      {stage === "planTest" && user && (
        <PlanTest
          ...
          onComplete={async (result) => {
            ...
            // 在问答题完成后立即生成背景图（map样式，中间水平线位置留空）
            const favorite = result.favoriteTopic.trim() || "a magical story world"
            const prompt = `A fantasy map-style background image featuring ${favorite}. Map illustration style, top-down view, beautiful fantasy landscape with elements from the theme. The center horizontal line area (around 50% from top, about 200-300 pixels height) should be kept relatively clear and minimal, avoiding major visual elements in that area as it will be used for task markers. No paths, roads, or routes. No text labels, just a beautiful map-style background image.`
      ...
      {stage === "bookReviewWriting" && user && bookReviewState.structure && bookReviewState.bookTitle && (
        <>
          {journeyActive && journeySelection && (
            <Lanyard difficulty={journeySelection.difficulty} />
          )}
          <BookReviewWriting
            reviewType={bookReviewState.reviewType!}
            bookTitle={bookReviewState.bookTitle}
