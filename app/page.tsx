      ...
      {stage === "journeyTicket" && user && writingAssessment && (
        <JourneyTicket
          ...
          onStart={async ({ type, difficulty }) => {
            ...
            // 生成背景图片的prompt（不再要求游戏地图和任务点空位）
            const typeName = type === "story" ? "story writing" : type === "bookReview" ? "book review writing" : "letter writing"
            const prompt = `A whimsical illustrated fantasy background inspired by ${favorite}, designed for ${typeName} journey. Colorful, kid-friendly, beautiful fantasy landscape, no text labels, just a beautiful background image.`

            // 设置地图状态为loading（不跳转，等待图片生成完成）
