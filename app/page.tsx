...
import DramaWelcome from "@/components/stages/drama-welcome"
import DramaWriting from "@/components/stages/drama-writing"
import DramaComplete from "@/components/stages/drama-complete"
...
export default function Home() {
  ...
  const [dramaState, setDramaState] = useState<DramaState>({
    background: {},
    scenes: [],
    characters: [],
  })
    ...
    <main className="min-h-screen">
      ...
      {stage === "writeTypeSelection" && user && (
        <WriteTypeSelection
          ...
          onSelectDrama={() => setStage("dramaWelcome")}
      ...
      {/* Drama Writing Flow */}
      {/* Welcome页面只在从writeTypeSelection进入时显示，不在journeyMap中显示 */}
      {stage === "dramaWelcome" && user && !journeyActive && (
        <DramaWelcome
          language={language}
          onStartDrama={() => {
            setDramaState({
      ...
      {stage === "dramaWriting" && user && (
        <DramaWriting
          ...
          onComplete={(dramaData) => {
            setDramaState(dramaData)
            setStage(journeyActive ? "journeyMap" : "dramaComplete")
          }}
          onBack={() => setStage(journeyActive ? "journeyMap" : "dramaWelcome")}
        />
      )}

      {stage === "dramaComplete" && user && dramaState.scenes.length > 0 && (
        <DramaComplete
          background={dramaState.background}
          scenes={dramaState.scenes}
          ...
          onBack={() => setStage(journeyActive ? "journeyMap" : "dramaWriting")}
          userId={user.username}
          workId={editingWorkId}
        />
      )}
