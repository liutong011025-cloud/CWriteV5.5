          ...
          onReset={async () => {
            ...
            if (journeyActive && user && currentPin && bookReviewState.review.trim().length > 0) {
              try {
                ...
                const previous = mapImageUrl || "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII="
          ...
          onReset={async () => {
            ...
            if (journeyActive && user && currentPin && storyState.story.trim().length > 0) {
              try {
                ...
                // 2. 調用地圖升級 API（初次沒有圖時使用一個中性灰色 base64 圖當作底圖）
                const previous = mapImageUrl || "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII="
